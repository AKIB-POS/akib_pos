...
class _ExpiredStockPageState extends State<ExpiredStockPage> {
  ...
  Future<void> _fetchExpiredStock() async {
    final branchId = _authSharedPref.getBranchId() ?? 0;
    await context.read<ExpiredStockCubit>().fetchExpiredStock(branchId: branchId);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        titleSpacing: 0,
        title: const Text('Stok Kedaluwarsa', style: AppTextStyle.headline5),
      ),
      body: RefreshIndicator(
        color: AppColors.primaryMain,
        onRefresh: _fetchExpiredStock,
        child: BlocBuilder<ExpiredStockCubit, ExpiredStockState>(
          builder: (context, state) {
            if (state is ExpiredStockLoading) {
              return _buildLoadingList();
            } else if (state is ExpiredStockLoaded) {
              if (state.expiredStock.expiredStocks.isEmpty) {
                return Utils.buildEmptyStatePlain(
                  "Tidak Ada Stok Kadaluarsa",
                  "Tidak ada stok yang telah kedaluwarsa untuk saat ini.",
                );
              } else {
                return _buildExpiredStockList(state.expiredStock.expiredStocks);
              }
            } else if (state is ExpiredStockError) {
              return Utils.buildErrorStatePlain(
                title: "Gagal Memuat Data",
                message: state.message,
                onRetry: _fetchExpiredStock,
              );
            } else {
  ...
  Widget _buildExpiredStockList(List<ExpiredStock> expiredStocks) {
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: expiredStocks.length,
      itemBuilder: (context, index) {
        final stock = expiredStocks[index];
        return _buildExpiredStockItem(stock);
      },
    );
  }

  Widget _buildExpiredStockItem(ExpiredStock stock) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildVendorAndExpiryColumn(vendor, expiryDate),
          ],
        ),
      ),
    );
  ...
  Widget _buildVendorAndExpiryColumn(String vendor, String expiryDate) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _buildInfoRow('Tanggal Kedaluwarsa', expiryDate),
        ],
      ),
    );
  }
