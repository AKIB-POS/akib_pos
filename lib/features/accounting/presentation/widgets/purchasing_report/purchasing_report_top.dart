                    ...
                    children: [
                      BlocBuilder<DateRangePurchaseCubit, String>(
                        builder: (context, state) {
                          // Use the same logic as in SalesReportTop to format the date
                          final dates = state.split(' - ');
                          final startDate = DateTime.parse(dates[0]);
                          final endDate = DateTime.parse(dates[1]);

                          // Displayed date logic (reused from SalesReportTop)
                          String displayedDate;
                          if (dates[0] == dates[1]) {
                            displayedDate = DateFormat('d MMMM yyyy', 'id').format(startDate);
                          } else {
                            displayedDate = "${DateFormat('d MMMM yyyy', 'id').format(startDate)} - ${DateFormat('d MMMM yyyy', 'id').format(endDate)}";
                          }
