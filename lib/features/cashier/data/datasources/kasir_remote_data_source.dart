...
abstract class KasirRemoteDataSource {
  ...
  Future<List<ProductModel>> searchProductsByName(String productName);
}
...
class KasirRemoteDataSourceImpl implements KasirRemoteDataSource {
  ...
  Future<List<SubCategoryModel>> getSubCategories(int categoryId) async {
    final response = await _getFromUrl('${URLs.baseUrlMock}/sub-categories/$categoryId');
    return _parseSubCategoryList(response);
  }

  @override
  Future<List<ProductModel>> getProductsByCategory(int categoryId) async {
    final response = await _getProductsFromUrl('${URLs.baseUrlMock}/products/categories/$categoryId');
    return _parseProductList(response);
  }

  @override
  Future<List<ProductModel>> getProductsBySubCategory(int subCategoryId) async {
    final response = await _getProductsFromUrl('${URLs.baseUrlMock}/products/sub-categories/$subCategoryId');
    return _parseProductList(response);
  }

  @override
  Future<List<ProductModel>> searchProductsByName(String productName) async {
    final response = await _getProductsFromUrl('${URLs.baseUrlMock}/products?search=$productName');
    return _parseProductList(response);
  }
